 ...
 * Module: Debate System v2.7 (Full Integrity)
 * Final Refactor: No-Grease / Zero-Trust
 */
...
const POSTURAS_CONFIG = {
  'debate-libre-albedrio': [
    ...
    { id: 'libertarismo', nombre: 'Libertarismo filosófico', desc: 'Somos genuinamente libres más allá de las causas que nos precedieron.', color: '#34D399', colorBg: 'rgba(52,211,153,0.08)', colorBorder: 'rgba(52,211,153,0.25)' },
...
const PREGUNTAS_EXAMEN = [
  { texto: '¿Qué debes hacer al responder el argumento de otro?', opciones: ['Ignorar', 'Citar fragmento', 'Resumir', 'Preguntar'], correcta: 1 },
...
async function initDebate() {
  console.log("%c[Rafa OS] Iniciando motor de debate...", "color: #C4A8E8; font-weight: bold;");
  ...
  if (!state.debateId) {
    console.error("[Rafa OS] ID de debate no mapeado.");
    return;
  }

  const { data: { user } } = await state.db.auth.getUser();
  if (!user) {
    console.warn("[Rafa OS] Usuario no logueado.");
    return;
  }
  state.user = user;
  ...
  // Títulos Dinámicos
  const titulos = { 'debate-libre-albedrio': 'El Libre Albedrío No Es Libertad', 'debate-moral': 'La Moral que Nadie Eligió' };
...
async function query(promise) {
  ...
  if (error) {
    console.error(`[DB Error]: ${error.code}`, error);
...
function renderBurbujas() {
  ...
  if (!container) return console.error("[Rafa OS] No se encontró el contenedor 'debate-burbujas'");
...
async function enviarExamen() {
  ...
  nota.textContent = 'Criterio validado. Entrando...';
...
async function cargarMensajes() {
  ...
  if (!mensajes?.length) return container.innerHTML = `<div class="debate-vacio">Sé el primero en romper el silencio.</div>`;
...
async function renderMensaje(m, esResp) {
  ...
  return `
    <div class="debate-mensaje ${esResp ? 'es-respuesta' : ''}">
      <div class="debate-mensaje-header">
        ...
        <span class="debate-mensaje-nombre" style="color:${postura?.color || '#fff'}">${p.username}</span>
      </div>
...
function activarProtocoloBufon() { 
  document.getElementById('bufon-modal')?.classList.remove('oculto'); 
  const wrap = document.getElementById('debate-input-wrap');
  if (wrap) wrap.innerHTML = '<p class="bufon-msg">Tu voz ha sido silenciada permanentemente.</p>'; 
}
